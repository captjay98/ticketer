...
<template>
    <AuthenticatedLayout>
        <div class="flex flex-col justify-center mt-4 w-screen h-screen">
            ...
            <div
                class="flex overflow-auto flex-wrap gap-6 justify-center items-center"
            >
                <div
                ...
                >
                    <h4
                        class="m-auto mt-4 w-11/12 text-xl font-semibold text-center"
                    >
                        {{ oneSeat?.coach?.coach_name }}
                    </h4>

                    <div v-if="oneSeat?.coach?.coach_class === 'Economy'" class="grid grid-cols-6 gap-3 justify-center items-center px-1 pt-10 pb-4 bg-green-100 max-sm:gap-2">
                        <div v-for="seat in seats" :key="seat.id" class="flex justify-center items-center py-2 px-2 w-6 h-7 text-sm bg-green-300 rounded-b-sm rounded-t-lg odd:mx-3 max-sm:w-[1.2rem] max-sm:h-[1.5rem]"
                            :class="{
                    ...
                    <div v-if="oneSeat?.coach?.coach_class === 'Business'" class="grid grid-cols-4 gap-3 items-center px-2 pt-10 pb-4 bg-green-100 max-sm:gap-2"
                    >
                        <div v-for="seat in seats" :key="seat.id" class="flex justify-center items-center py-2 px-2 w-6 h-7 text-sm bg-green-300 rounded-b-sm rounded-t-lg odd:mx-5 max-sm:w-[1.2rem] max-sm:h-[1.5rem]"
                            :class="{
                                'bg-white shadow-2xl': selectedSeat === seat.id,
                                'bg-red-500/80 shadow-2xl disabled': seat.status === 'booked',
                                'bg-yellow-500/70 shadow-2xl disabled':seat.status === 'reserved',
